...
{
	std::vector<line_idx> inputs;//存储了门的输入的索引
	...
	line_idx p_output = ensure_line(output_name);//门的输出的index
	...
	{
		m_lines[m_gates[gate].get_inputs().at(i)].connect_as_input(gate);//在输入线的destination_gates中，插入门的索引
	}
...
line_idx CircuitGraph::ensure_line(const std::string& name)
{
	...
	//not exist,create a Line
	m_lines.emplace_back();//插入一个元素，使用Line()初始化
	//add_input add_output均使用ensure_line进行添加 故index会呈现增大的趋势
